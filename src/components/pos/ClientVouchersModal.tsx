...
export default function ClientVouchersModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" dir="rtl">
      ...
      {/* Panel */}
      <div className="relative z-10 w-full max-w-md mx-4 bg-zinc-900 border border-zinc-700/60 rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-white">مكافآت النقاط</p>
        ...
        <div className="p-4 max-h-[70vh] overflow-y-auto space-y-2.5">
          {loading ? (
            ...
            <div className="py-10 text-center">
              ...
              <p className="text-sm text-zinc-500">لا توجد مكافآت نشطة لهذا العميل</p>
            </div>
          ) : (
            vouchers.map(v => {
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex items-start gap-3 flex-1 min-w-0">
                      ...
                      <div className="min-w-0">
                        <p className="text-sm font-medium text-white truncate">{v.item.nameAr}</p>
                        ...
                        <div className="flex items-center gap-2 mt-1.5">
                          <code className="text-[10px] text-zinc-500 bg-zinc-800 px-1.5 py-0.5 rounded font-mono">
                            {v.voucherCode}
                          </code>
                          {v.expiresAt && (
                            <span className={`text-[10px] flex items-center gap-1 ${
                              v.isExpiringSoon ? 'text-orange-400' : 'text-zinc-500'
                            }`}>
                    ...
                    {/* Right: action */}
                    {used ? (
                      <div className="flex items-center gap-1 text-emerald-400 text-xs shrink-0 mt-1">
                      ...
                      <button
                        ...
                        className="shrink-0 mt-1 px-3 py-1.5 rounded-lg bg-yellow-500 hover:bg-yellow-400 text-black text-xs font-bold transition-colors disabled:opacity-50"
                      >
                        تطبيق
                      </button>
                    ) : null}
                  ...
                  {v.isExpiringSoon && !used && (
                    <div className="mt-2 flex items-center gap-1.5 text-[10px] text-orange-400 bg-orange-500/10 rounded-lg px-2.5 py-1.5">
        ...
        {vouchers.length > 0 && (
          <div className="px-5 py-3 border-t border-zinc-800 flex items-center justify-between">
            ...
            <button onClick={onClose} className="text-xs text-zinc-400 hover:text-white transition-colors">
              إغلاق
            </button>
          </div>
        )}
