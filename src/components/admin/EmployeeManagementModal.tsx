...
import { 
  ...
  Save, Plus, Edit2, Trash2, Loader2, CheckCircle2, AlertCircle, Copy
} from 'lucide-react';
...
export default function EmployeeManagementModal({ 
  ...
  const DEFAULT_SCHEDULE: WorkScheduleItem[] = [0, 1, 2, 3, 4, 5, 6].map(day => ({
    DayOfWeek: day,
    IsWorkingDay: day !== 5,
    StartTime: day !== 5 ? '09:00' : '',
    EndTime: day !== 5 ? '17:00' : '',
    BreakStartTime: '',
    BreakEndTime: '',
    Notes: day === 5 ? 'جمعة - إجازة أسبوعية' : ''
  }));
  ...
  const loadEmployeeData = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        const sched = Array.isArray(data.schedule) && data.schedule.length === 7
          ? data.schedule
          : DEFAULT_SCHEDULE;
        setSchedule(sched);
              ...
              <div className="bg-slate-800/50 rounded-xl p-10 border border-slate-700">
                ...
                <div className="grid grid-cols-2 gap-8">
                  ...
                  <div>
                    ...
                    <Select
                      value={profile.Job || ''}
                      onValueChange={(value) => setProfile({...profile, Job: value})}
                    >
                      <SelectTrigger className="bg-slate-700/50 border-slate-600 text-white placeholder:text-slate-400 focus:border-amber-500 focus:ring-amber-500/20 h-12 text-lg">
                        <SelectValue placeholder="اختر الوظيفة" />
                      </SelectTrigger>
                      <SelectContent className="bg-slate-800 border-slate-600">
                        <SelectItem value={JobType.BARBER} className="text-white">{JobType.BARBER}</SelectItem>
                        <SelectItem value={JobType.SKIN_CARE} className="text-white">{JobType.SKIN_CARE}</SelectItem>
                        <SelectItem value={JobType.ASSISTANT} className="text-white">{JobType.ASSISTANT}</SelectItem>
                        <SelectItem value={JobType.ADMINISTRATIVE} className="text-white">{JobType.ADMINISTRATIVE}</SelectItem>
                        <SelectItem value={JobType.MANAGER} className="text-white">{JobType.MANAGER}</SelectItem>
                        <SelectItem value={JobType.RECEPTIONIST} className="text-white">{JobType.RECEPTIONIST}</SelectItem>
                        <SelectItem value={JobType.BEAUTICIAN} className="text-white">{JobType.BEAUTICIAN}</SelectItem>
                        <SelectItem value={JobType.MASSAGE_THERAPIST} className="text-white">{JobType.MASSAGE_THERAPIST}</SelectItem>
                        <SelectItem value={JobType.NAIL_TECHNICIAN} className="text-white">{JobType.NAIL_TECHNICIAN}</SelectItem>
                        <SelectItem value={JobType.MAKEUP_ARTIST} className="text-white">{JobType.MAKEUP_ARTIST}</SelectItem>
                        <SelectItem value={JobType.HAIR_STYLIST} className="text-white">{JobType.HAIR_STYLIST}</SelectItem>
                        <SelectItem value={JobType.ESTHETICIAN} className="text-white">{JobType.ESTHETICIAN}</SelectItem>
                        <SelectItem value={JobType.OTHER} className="text-white">{JobType.OTHER}</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                  <div>
                  ...
                  <table className="w-full border-collapse">
                    <thead>
                      <tr className="border-b border-slate-600">
                        ...
                        <th className="text-center p-4 text-slate-300 font-semibold">إجراءات</th>
                      </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr key={day.DayOfWeek} className={`border-b border-slate-700/50 hover:bg-slate-700/20 transition-colors ${!day.IsWorkingDay ? 'opacity-60' : ''}`}>
                          ...
                          <td className="p-4">
                            <Button
                              ...
                              disabled={!day.IsWorkingDay && !day.StartTime}
                              className="bg-amber-500/20 border-amber-500/50 text-amber-400 hover:bg-amber-500/30 disabled:opacity-50 disabled:cursor-not-allowed"
                            >
            ...
            <div className="space-y-4">
              ...
              <div className="bg-slate-800/50 rounded-lg p-4 border border-slate-700">
                ...
                <div className="text-sm text-slate-300">
                  <p><span className="font-medium">اليوم:</span> {dayNames[sourceDay.DayOfWeek]}</p>
                  {sourceDay.IsWorkingDay && (
                    <>
                      <p><span className="font-medium">وقت العمل:</span> {sourceDay.StartTime} - {sourceDay.EndTime}</p>
                      {sourceDay.BreakStartTime && sourceDay.BreakEndTime && (
                        <p><span className="font-medium">وقت الراحة:</span> {sourceDay.BreakStartTime} - {sourceDay.BreakEndTime}</p>
                      )}
                      {sourceDay.Notes && (
                        <p><span className="font-medium">ملاحظات:</span> {sourceDay.Notes}</p>
                      )}
                    </>
                  )}
                  {!sourceDay.IsWorkingDay && (
                      ...
                      >
                        <div className="text-right">
                          ...
                          <div className="text-xs opacity-70">
                            {day.IsWorkingDay ? `${day.StartTime || '--'} - ${day.EndTime || '--'}` : 'إجازة'}
                          </div>
                        </div>
                      </Button>
                    ))}
