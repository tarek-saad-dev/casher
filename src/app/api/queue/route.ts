...
async function getEstimateColsSelect(db: Awaited<ReturnType<typeof getPool>>): Promise<string> {
...
export async function GET(req: NextRequest) {
  try {
    ...
    const date   = searchParams.get("date")   || new Date().toLocaleDateString('en-CA', { timeZone: 'Africa/Cairo' });
    ...
    if (empId)  { request.input("empId",  sql.Int,      parseInt(empId));  where += " AND qt.EmpID = @empId"; }
    if (status && status !== "all") { request.input("status", sql.NVarChar, status); where += " AND qt.Status = @status"; }
    ...
    return NextResponse.json({ error: "فشل تحميل قائمة الانتظار" }, { status: 500 });
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const {
      clientId, empId, notes, priority = 0, bookingId = null,
      services = [],
      ...
      services?: Array<{ proId?: number; proName?: string; qty?: number; price?: number; durationMinutes?: number }>;
    ...
    `);
    const found = tableCheck.recordset.map((r: { TABLE_NAME: string }) => r.TABLE_NAME);
    if (!found.includes('QueueTickets')) {
      console.error("[queue POST] QueueTickets table not found in current database");
      return NextResponse.json({ error: "جدول التذاكر غير موجود — يرجى تشغيل الميجرشين" }, { status: 503 });
    }
    ...
    const createdTime = now2.toLocaleTimeString('en-GB', { timeZone: 'Africa/Cairo', hour12: false });
    ...
    const serviceProIds = services.map(s => s.proId).filter((id): id is number => !!id);
    const customerDur = await getServicesDuration(db, serviceProIds, defaultDur);
    ...
    if (empId) {
      const qIvs = await buildQueueIntervals(db, empId, today, now2, defaultDur);
      ...
      finalEstWait   = Math.max(0, Math.round((finalEstStart.getTime() - now2.getTime()) / 60000));
      finalWaitCount = qIvs.length;
      console.log('[queue create] final estimatedStartTime saved', finalEstStart.toISOString(), 'waitMin', finalEstWait, 'queueBefore', finalWaitCount);
    }
    ...
    try {
      ...
      if (empId && finalEstStart) {
        const qIvsTx = await buildQueueIntervals(db, empId, today, now2, defaultDur);
        const bIvsTx = await buildBookingIntervals(db, empId, today, defaultDur);
        const allIvsTx = [...qIvsTx, ...bIvsTx].sort((a, b) => a.start.getTime() - b.start.getTime());
        const verifiedSlot = findFirstFreeSlot(now2, customerDur, allIvsTx);
        // Only update if slot changed significantly (> 1 min diff)
        if (Math.abs(verifiedSlot.getTime() - finalEstStart.getTime()) > 60000) {
          finalEstStart  = verifiedSlot;
          finalEstWait   = Math.max(0, Math.round((verifiedSlot.getTime() - now2.getTime()) / 60000));
          finalWaitCount = qIvsTx.length;
          console.log('[queue create] slot adjusted inside tx', finalEstStart.toISOString());
        }
      }
      ...
      const colCheck = await transaction.request().query(`
        ...
        WHERE TABLE_NAME = 'QueueTickets'
          AND COLUMN_NAME IN ('EstimatedStartTime','EstimatedWaitMinutes','WaitingCountAtCreation')
      `);
      const existingCols = new Set(colCheck.recordset.map((r: { COLUMN_NAME: string }) => r.COLUMN_NAME));
      ...
      if (hasEst)    insReq.input('estStart',  sql.DateTime2, finalEstStart ?? null);
    ...
    await db.request()
      ...
      `).catch((e: unknown) => console.error("[queue POST] history write failed", e));

    // Save ticket services (non-critical — outside transaction)
    if (services.length > 0) {
      console.log('[queue POST] saving services', services.map(s => ({ proId: s.proId, proName: s.proName })));
      ...
      if (svcTableCheck.recordset.length) {
        for (const svc of services) {
          await db.request()
            ...
            `)
            .catch((e: unknown) => console.error('[queue POST] service insert failed', e));
        }
        ...
        console.log('[queue POST] db services after insert', svcCheck.recordset);
      } else {
        console.warn('[queue POST] QueueTicketServices table does not exist — services not saved');
      }
    } else {
    ...
    const fullTicketRes = await db.request()
      ...
      `).catch((e: unknown) => { console.error('[queue POST] re-fetch failed', e); return { recordset: [] as any[] }; });
    ...
    const latestRes = await db.request().query(`
      SELECT TOP 5
    ...
    let savedServices: Array<{ ProID: number | null; ProName: string | null; Qty: number }> = [];
    const svcFetchRes = await db.request()
      .input('tid', sql.Int, newId)
      .query(`
    ...
    const servicesText = savedServices
      .map(s => s.ProName ?? '').filter(Boolean).join(' + ');
    ...
    return NextResponse.json({ error: "فشل إنشاء رقم الانتظار" }, { status: 500 });
  }
}
