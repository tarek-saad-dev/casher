 ...
 */
export async function GET(req: NextRequest): Promise<NextResponse<LoyaltyActivityListResponse | LoyaltyErrorResponse>> {
  try {
    ...
    const countRequest = db.request().input('clientLoyaltyId', sql.Int, clientLoyaltyId);
    ...
    const request = db.request().input('clientLoyaltyId', sql.Int, clientLoyaltyId);
    ...
    const activity: LoyaltyActivityItem[] = result.recordset.map(mapLedgerToActivityItem);
