...
/**
 * POST /api/admin/hr/payroll-gap-review/assign-attendance
 * Body: { empId, branchId, year, month, workDate, options? }
 */
export async function POST(req: NextRequest) {
  try {
    ...
    const options = (body.options ?? {}) as Pick<
      PayrollGapApplyOptions,
      'defaultCheckoutTime' | 'notesPrefix'
    >;
    ...
    const result = await assignEmployeePayrollGapDayAttendance({
      empId,
      branchId,
      year,
      month,
    ...
    console.error('[api/admin/hr/payroll-gap-review/assign-attendance] POST error:', message);
