 ...
 */
export async function getEmployeeAvailabilityBase(
  empId: number,
  date: string,
  ...
  try {
    ...
    const empCheck = await db
      ...
      .query(
        "SELECT EmpID, EmpName, isActive FROM dbo.TblEmp WHERE EmpID = @empId",
      );
    ...
    const dayOffCheck = await db
      .request()
      .input("empId", sql.Int, empId)
      .input("offDate", sql.Date, date).query(`
...
export async function getEmployeeAvailabilityAtTime(
  ...
  if (
    baseAvailability.schedule?.StartTime &&
    baseAvailability.schedule?.EndTime
  ) {
...
-- DayOfWeek should be passed as parameter (0=Sunday, 6=Saturday)
-- JS: new Date(date).getDay() → 0-6
-- Use DATEDIFF for consistent calculation regardless of SQL DATEFIRST:
DECLARE @DayOfWeek TINYINT = (DATEDIFF(DAY, '19000107', @CheckDate) % 7); -- 0 = Sunday, 6 = Saturday
