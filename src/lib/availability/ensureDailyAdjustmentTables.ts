/**
 * Phase 3A — Idempotent ensure for TblEmpDailyAdjustment (+ Window).
 * Do not call from SERIALIZABLE booking transactions (DDL/pool only).
 */
...
async function tablesExist(db: ConnectionPool): Promise<boolean> {
  const result = await db.request().query(`
    SELECT
      CASE WHEN OBJECT_ID(N'dbo.TblEmpDailyAdjustment', N'U') IS NOT NULL THEN 1 ELSE 0 END AS hasAdj,
      CASE WHEN OBJECT_ID(N'dbo.TblEmpDailyAdjustmentWindow', N'U') IS NOT NULL THEN 1 ELSE 0 END AS hasWin
  `);
  const row = result.recordset[0];
  return Number(row?.hasAdj) === 1 && Number(row?.hasWin) === 1;
}

export async function ensureDailyAdjustmentTables(
  db?: ConnectionPool,
): Promise<boolean> {
  ...
  try {
    ...
    console.warn('[ensureDailyAdjustmentTables] create failed:', err);
  }

  try {
    tablesReady = await tablesExist(pool);
    if (!tablesReady) {
      console.warn('[ensureDailyAdjustmentTables] tables still missing after ensure');
    }
    return tablesReady;
  } catch (err) {
    console.warn('[ensureDailyAdjustmentTables] existence check failed:', err);
    tablesReady = false;
    return false;
  }
}
...
export function __resetDailyAdjustmentTablesCacheForTests(): void {
  tablesReady = null;
}
