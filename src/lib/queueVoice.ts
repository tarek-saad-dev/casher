...
function resolveFields(ticket: QueueTicketVoiceInfo) {
  const code = (ticket.TicketCode ?? ticket.ticketCode ?? "").trim();
...
export function buildQueueAnnouncement(ticket: QueueTicketVoiceInfo): string {
  ...
  if (client && barber)
    return `عميلنا ${client}، يتفضل يتوجه إلى الأستاذ ${barber}.`;
  if (client) return `عميلنا ${client}، يتفضل يتوجه إلى منطقة الخدمة.`;
  if (code && barber)
    return `صاحب الدور رقم ${code}، يتفضل يتوجه إلى الأستاذ ${barber}.`;
...
export async function speakWithBrowser(
  ...
  console.log(`[voice] fallback to browser (${locale})`);
  window.speechSynthesis.cancel();

  const voices = await loadBrowserVoices();
  const voice = isEnglish ? pickEnglishVoice(voices) : pickArabicVoice(voices);
  if (!voice)
    console.warn(
      `[voice] no ${isEnglish ? "English" : "Arabic"} voice found, using browser default`,
    );
  ...
  return new Promise<void>((resolve, reject) => {
    utterance.onend = () => {
      console.log("[voice] browser done");
      resolve();
    };
    utterance.onerror = (e) => {
