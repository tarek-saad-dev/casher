...
/**
 * Load وقت مستقطع intervals for many employees on one work date.
 * Keyed by EmpID.
 */
export async function loadBreaksByEmpIdsOnWorkDate(
  db: DbLike,
  workDate: string,
  empIds: number[],
): Promise<Map<number, AttendanceBreakInterval[]>> {
  const map = new Map<number, AttendanceBreakInterval[]>();
  const idList = [...new Set(empIds.map((id) => Number(id)).filter((id) => id > 0))];
  if (idList.length === 0) return map;
  ...
  const result = await db
    ...
    .query(`
      SELECT
        ...
        b.Notes
      FROM dbo.TblEmpAttendanceBreak b
      INNER JOIN dbo.TblEmpAttendance a ON a.ID = b.AttendanceID
      WHERE a.WorkDate = @workDate
        AND a.EmpID IN (${idList.join(',')})
      ORDER BY a.EmpID, b.LeaveAt, b.ID
    `);
