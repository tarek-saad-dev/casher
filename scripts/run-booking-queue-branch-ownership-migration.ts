#!/usr/bin/env npx tsx
/**
 * Phase 1F booking/queue branch-ownership migration runner.
...
const MIGRATION_NAME = 'add-booking-queue-branch-ownership.sql';
...
async function main() {
  ...
  console.log('Phase 1F migration runner');
  ...
  if (!confirmMaintenance) {
    console.error('Refusing: pass --confirm-maintenance (keep app/nightly writers paused).');
    process.exit(1);
  }
  ...
  const captureBefore = spawnSync(
    process.execPath,
    [path.join(__dirname, 'audit-branches/10-capture-phase1f-booking-queue-state.cjs'), 'before'],
  ...
  try {
    ...
    `);
    console.log(`  open shift count: ${Number(openShifts.recordset[0].OpenShiftCount)}`);
    console.log('  Continuing under explicit --confirm-maintenance');
  ...
  const captureAfter = spawnSync(
    process.execPath,
    [path.join(__dirname, 'audit-branches/10-capture-phase1f-booking-queue-state.cjs'), 'after'],
    { stdio: 'inherit', cwd: path.join(__dirname, '..'), env: process.env },
  );
  ...
  const verify = spawnSync(
    ...
    [
      'tsx',
      path.join(__dirname, 'verify-booking-queue-branch-ownership.ts'),
  ...
  console.log('Phase 1F migration + verification OK');
}
