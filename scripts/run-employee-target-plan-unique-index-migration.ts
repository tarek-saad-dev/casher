...
async function main() {
  ...
  const text = fs.readFileSync(
    path.join(
      __dirname,
      '..',
      'db/migrations/add-employee-target-plan-effective-date-unique-index.sql',
    ),
    'utf8',
  );
  ...
  const check = await pool.request().query(`
    ...
    FROM sys.indexes i
    WHERE i.object_id = OBJECT_ID(N'dbo.TblEmpTargetPlan')
      AND i.name = N'UX_TblEmpTargetPlan_EmpID_EffectiveFrom'
  `);
