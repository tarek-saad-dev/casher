...
/**
 * Runs db/migrations/add-employee-ledger-tip-reason.sql
 * Adds tip to CK_TblEmpLedgerEntry_EntryReason (idempotent).
 */
...
const MIGRATION_FILE = path.join(
  ...
  'db',
  'migrations',
  'add-employee-ledger-tip-reason.sql',
);
...
async function main() {
  ...
  console.log(`Running ${batches.length} batch(es) from add-employee-ledger-tip-reason.sql ...\n`);
  ...
  const allowed = definition.includes('tip');
  console.log('\nConstraint definition:', definition);
  console.log(allowed ? '✓ tip is allowed' : '✗ tip still missing');
