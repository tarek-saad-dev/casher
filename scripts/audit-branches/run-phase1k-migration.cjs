/**
 * Apply Phase 1K attendance BranchID migration to cloud last132.
 */
...
async function main() {
  const file = path.join(
    ...
    'db',
    'migrations',
    'add-attendance-branch-ownership.sql',
  );
  ...
  try {
    ...
    console.log(`[phase1k] applying ${batches.length} batches to ${db}`);
    ...
    console.log('[phase1k] migration complete');
  } finally {
    await pool.close();
  }
}
