/**
 * Apply Phase 1J SQL migration to cloud last132 (writable).
 * Usage: node scripts/audit-branches/run-phase1j-migration.cjs
 */
...
async function main() {
  const file = path.join(
    ...
    'db',
    'migrations',
    'add-branch-inventory-and-purchase-ownership.sql',
  );
  ...
  try {
    ...
    console.log(`[phase1j] applying ${batches.length} batches to ${db}`);
    ...
    console.log('[phase1j] migration complete');
  } finally {
    await pool.close();
  }
}
