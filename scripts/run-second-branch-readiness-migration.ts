#!/usr/bin/env npx tsx
/**
 * Phase 1G second-branch readiness migration runner.
 * Default: cloud / last132. Never prints secrets.
 * Adds BranchName/ShortName uniqueness only — does not create a second branch.
 */
...
const MIGRATION_NAME = 'add-second-branch-readiness.sql';
...
async function main() {
  ...
  console.log('Phase 1G second-branch readiness migration runner');
  ...
  console.log('  NOTE: this migration only adds BranchName/ShortName uniqueness — no data changes');
  ...
  const verify = spawnSync(
    ...
    [
      'tsx',
      path.join(__dirname, 'verify-second-branch-readiness.ts'),
  ...
  console.log('Phase 1G migration + verification OK');
}
