...
async function main() {
  ...
  const smokeBlockers = r.blockers.filter((b) => b.requiredFor.includes('smoke'));
  console.log(
    JSON.stringify(
      {
        isReadyForSmoke: r.isReadyForSmoke,
        isReadyForInternalLive: r.isReadyForInternalLive,
        smokeBlockers: smokeBlockers.map((b) => ({ key: b.key, details: b.details })),
      },
      null,
      2,
    ),
  );
  writeFileSync(join(__dirname, '_phase1s-smoke-blockers.json'), JSON.stringify(smokeBlockers, null, 2));
  process.exit(0);
}
main().catch((e) => {
  console.error(e);
