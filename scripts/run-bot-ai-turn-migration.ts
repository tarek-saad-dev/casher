#!/usr/bin/env npx tsx
/**
 * Apply Phase 3 TblBotAiTurn schema.
 */
...
async function main() {
  ...
  const tables = await pool.request().query(`
    SELECT name
    FROM sys.tables
    WHERE name = N'TblBotAiTurn'
  `);
  console.table(tables.recordset);
  if ((tables.recordset as Array<{ name: string }>).length !== 1) {
    throw new Error('TblBotAiTurn was not created');
  }

  await closePool();
  console.log('Phase 3 AI turn schema migration OK');
}
