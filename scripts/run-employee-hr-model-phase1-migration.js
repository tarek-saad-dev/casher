#!/usr/bin/env node
/* eslint-disable no-console */
/**
 * Runs db/migrations/add-employee-employment-type-payroll-method.sql
 * Safe to re-run (idempotent columns, constraints, backfill).
 */

const sql = require('mssql');
const dotenv = require('dotenv');
const fs = require('fs');
const path = require('path');

dotenv.config({ path: path.join(__dirname, '..', '.env') });
dotenv.config({ path: path.join(__dirname, '..', '.env.local') });

const config = {
  server: process.env.CLOUD_DB_SERVER || process.env.DB_SERVER || '',
  port: parseInt(process.env.CLOUD_DB_PORT || process.env.DB_PORT || '1433', 10),
  database: process.env.CLOUD_DB_NAME || process.env.DB_DATABASE || 'HawaiRestaurant',
  user: process.env.CLOUD_DB_USER || process.env.DB_USER || '',
  password: process.env.CLOUD_DB_PASSWORD || process.env.DB_PASSWORD || '',
  options: {
    encrypt: process.env.CLOUD_DB_ENCRYPT === 'true' || process.env.DB_ENCRYPT === 'true',
    trustServerCertificate:
      process.env.CLOUD_DB_TRUST_CERT === 'true' ||
      process.env.DB_TRUST_SERVER_CERTIFICATE === 'true',
    enableArithAbort: true,
  },
  connectionTimeout: 60000,
  requestTimeout: 120000,
};

const MIGRATION_FILE = path.join(
  __dirname,
  '..',
  'db',
  'migrations',
  'add-employee-employment-type-payroll-method.sql',
);

function splitSqlBatches(content) {
  return content
    .split(/^\s*GO\s*$/im)
    .map((b) => b.trim())
    .filter(Boolean);
}

async function columnExists(pool, columnName) {
  const r = await pool.request().query(`
    SELECT 1 AS ok
    FROM INFORMATION_SCHEMA.COLUMNS
    WHERE TABLE_SCHEMA = N'dbo' AND TABLE_NAME = N'TblEmp'
      AND COLUMN_NAME = N'${columnName.replace(/'/g, "''")}'
  `);
  return r.recordset.length > 0;
}

async function printSummary(pool) {
  console.log('\n── Post-migration column check ──');
  for (const col of ['EmploymentType', 'PayrollMethod', 'DailyRate', 'ManualHourlyRate']) {
    const exists = await columnExists(pool, col);
    console.log(`  ${col}: ${exists ? '✓ exists' : '✗ missing'}`);
  }

  const byEmpType = await pool.request().query(`
    SELECT ISNULL(EmploymentType, N'NULL') AS EmploymentType, COUNT(*) AS cnt
    FROM dbo.TblEmp GROUP BY EmploymentType ORDER BY EmploymentType
  `);
  console.log('\n── Counts by EmploymentType ──');
  console.table(byEmpType.recordset);

  const byPayMethod = await pool.request().query(`
    SELECT ISNULL(PayrollMethod, N'NULL') AS PayrollMethod, COUNT(*) AS cnt
    FROM dbo.TblEmp GROUP BY PayrollMethod ORDER BY PayrollMethod
  `);
  console.log('── Counts by PayrollMethod ──');
  console.table(byPayMethod.recordset);

  const nulls = await pool.request().query(`
    SELECT COUNT(*) AS cnt FROM dbo.TblEmp WHERE EmploymentType IS NULL OR PayrollMethod IS NULL
  `);
  console.log(`\nEmployees with NULL EmploymentType or PayrollMethod: ${nulls.recordset[0].cnt}`);

  const violations = await pool.request().query(`
    SELECT EmpID, EmpName, EmploymentType, PayrollMethod
    FROM dbo.TblEmp
    WHERE EmploymentType = N'freelance' AND PayrollMethod = N'monthly'
  `);
  if (violations.recordset.length > 0) {
    console.warn('\n⚠ freelance + monthly violations (should be 0 after migration):');
    console.table(violations.recordset);
  } else {
    console.log('\n✓ No freelance + monthly violations');
  }
}

async function main() {
  if (!config.server || !config.user) {
    console.error('Missing DB credentials. Set DB_SERVER / DB_USER / DB_PASSWORD in .env or .env.local');
    process.exit(1);
  }

  if (!fs.existsSync(MIGRATION_FILE)) {
    console.error('Migration file not found:', MIGRATION_FILE);
    process.exit(1);
  }

  const sqlText = fs.readFileSync(MIGRATION_FILE, 'utf8');
  const batches = splitSqlBatches(sqlText);

  console.log('Connecting to', config.server, '/', config.database, '...');
  const pool = await sql.connect(config);
  console.log('Connected.\n');
  console.log(`Running ${batches.length} batch(es) from add-employee-employment-type-payroll-method.sql ...\n`);

  let batchNum = 0;
  for (const batch of batches) {
    batchNum += 1;
    try {
      const result = await pool.request().query(batch);
      if (result.recordsets && result.recordsets.length > 0) {
        for (const rs of result.recordsets) {
          if (Array.isArray(rs) && rs.length > 0) {
            console.log(`\n[Batch ${batchNum} result]`);
            console.table(rs);
          }
        }
      }
    } catch (err) {
      console.error(`\n✗ Batch ${batchNum} failed:`, err.message);
      if (err.precedingErrors) {
        for (const pe of err.precedingErrors) {
          console.error('  preceding:', pe.message);
        }
      }
      await pool.close();
      process.exit(1);
    }
  }

  await printSummary(pool);
  await pool.close();
  console.log('\n✅ Phase 1 migration complete.');
  console.log('   Run: node scripts/audit-employee-hr-model-phase1.js');
}

main().catch((err) => {
  console.error(err.message || err);
  process.exit(1);
});
