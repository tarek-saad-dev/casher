#!/usr/bin/env npx tsx
/**
 * Phase 1C business-day/shift migration runner.
...
const MIGRATION_NAME = 'add-branch-business-day-and-shift.sql';
...
async function main() {
  ...
  console.log('Phase 1C migration runner');
  ...
  const captureBefore = spawnSync(
    process.execPath,
    [path.join(__dirname, 'audit-branches/07-capture-phase1c-state.cjs'), 'before'],
  ...
  try {
    const openShifts = await pool.request().query(`
      SELECT COUNT(*) AS c FROM dbo.TblShiftMove WHERE ISNULL(Status,0)=1
    `);
    const openDays = await pool.request().query(`
      SELECT COUNT(*) AS c FROM dbo.TblNewDay WHERE Status = 1
    `);
    console.log(`  open shifts: ${openShifts.recordset[0].c} (will backfill, not close)`);
    console.log(`  open days: ${openDays.recordset[0].c} (will backfill, not close)`);
  ...
  spawnSync(
    process.execPath,
    [path.join(__dirname, 'audit-branches/07-capture-phase1c-state.cjs'), 'after'],
    { stdio: 'inherit', cwd: path.join(__dirname, '..'), env: process.env },
  );
  ...
  const verify = spawnSync(
    ...
    [
      'tsx',
      path.join(__dirname, 'verify-branch-business-day-and-shift.ts'),
  ...
  console.log('Phase 1C migration + verification OK');
}
